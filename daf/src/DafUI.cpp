...
#if DAF_UI_FILE_BROWSER && !defined(DAF_OS_MAC)
# define DAF_PUGL_NAMESPACE_MACRO_HELPER(NS, SEP, FUNCTION) NS ## SEP ## FUNCTION
# define DAF_PUGL_NAMESPACE_MACRO(NS, FUNCTION) DAF_PUGL_NAMESPACE_MACRO_HELPER(NS, _, FUNCTION)
# define x_fib_add_recent          DAF_PUGL_NAMESPACE_MACRO(plugin, x_fib_add_recent)
# define x_fib_cfg_buttons         DAF_PUGL_NAMESPACE_MACRO(plugin, x_fib_cfg_buttons)
# define x_fib_cfg_filter_callback DAF_PUGL_NAMESPACE_MACRO(plugin, x_fib_cfg_filter_callback)
# define x_fib_close               DAF_PUGL_NAMESPACE_MACRO(plugin, x_fib_close)
# define x_fib_configure           DAF_PUGL_NAMESPACE_MACRO(plugin, x_fib_configure)
# define x_fib_filename            DAF_PUGL_NAMESPACE_MACRO(plugin, x_fib_filename)
# define x_fib_free_recent         DAF_PUGL_NAMESPACE_MACRO(plugin, x_fib_free_recent)
# define x_fib_handle_events       DAF_PUGL_NAMESPACE_MACRO(plugin, x_fib_handle_events)
# define x_fib_load_recent         DAF_PUGL_NAMESPACE_MACRO(plugin, x_fib_load_recent)
# define x_fib_recent_at           DAF_PUGL_NAMESPACE_MACRO(plugin, x_fib_recent_at)
# define x_fib_recent_count        DAF_PUGL_NAMESPACE_MACRO(plugin, x_fib_recent_count)
# define x_fib_recent_file         DAF_PUGL_NAMESPACE_MACRO(plugin, x_fib_recent_file)
# define x_fib_save_recent         DAF_PUGL_NAMESPACE_MACRO(plugin, x_fib_save_recent)
# define x_fib_show                DAF_PUGL_NAMESPACE_MACRO(plugin, x_fib_show)
# define x_fib_status              DAF_PUGL_NAMESPACE_MACRO(plugin, x_fib_status)
...
#if DAF_UI_WEB_VIEW && !defined(DAF_OS_MAC)
...
static double getDesktopScaleFactor(const uintptr_t parentWindowHandle)
{
    ...
    DAF_SAFE_ASSERT_RETURN(display != nullptr, 1.0);
    ...
    /* Wayland has no global, window-independent scale factor to query: the compositor only tells a
       ...
       or wp_fractional_scale_v1. That happens after this function is called, so return the existing
       "unknown" sentinel and let the Window follow puglGetScaleFactor() from configure events.
       DAF_SCALE_FACTOR above and a non-zero host-provided scale still override this path. */
    return 0.0;
   #endif
...
PluginWindow& UI::PrivateData::createNextWindow(UI* const ui, uint width, uint height)
{
    ...
    if (uiData->callbacksPtr != nullptr)
    {
        ...
        else
        {
            ...
            path.truncate(path.rfind(DAF_OS_SEP));
            path += "/resources";
        }
...
void UI::PrivateData::webViewMessageCallback(void* const arg, char* const msg)
{
    ...
    if (std::strncmp(msg, "setparam ", 9) == 0)
    {
        ...
        DAF_SAFE_ASSERT_RETURN(strvalue != nullptr && strindex != strvalue,);
    ...
    if (std::strncmp(msg, "setstate ", 9) == 0)
    {
        ...
        DAF_SAFE_ASSERT_RETURN(sep != nullptr,);
        *sep = '\0';
    ...
    if (std::strncmp(msg, "sendnote ", 9) == 0)
    {
        ...
        DAF_SAFE_ASSERT_RETURN(strnote != nullptr && strchannel != strnote,);

        const ulong note = std::strtoul(strnote, &strvelocity, 10);
        DAF_SAFE_ASSERT_RETURN(strvelocity != nullptr && strchannel != strvelocity,);

        const ulong velocity = std::strtoul(strvelocity, &end, 10);
        DAF_SAFE_ASSERT_RETURN(end != nullptr && strvelocity != end,);
...
UI::UI(const uint width, const uint height, const bool automaticallyScaleAndSetAsMinimumSize)
    : UIWidget(UI::PrivateData::createNextWindow(this,
               // width
              #ifdef DAF_UI_DEFAULT_WIDTH
               width == 0 ? DAF_UI_DEFAULT_WIDTH :
              #endif
               width,
               // height
              #ifdef DAF_UI_DEFAULT_HEIGHT
               height == 0 ? DAF_UI_DEFAULT_HEIGHT :
              #endif
               height
               )),
      uiData(UI::PrivateData::s_nextPrivateData)
{
    ...
    {
        Widget::setSize(DAF_UI_DEFAULT_WIDTH, DAF_UI_DEFAULT_HEIGHT);
    }
   #endif
}
